...
void NavigationBlock::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget)
{
	...
	linearGrad.setColorAt(1, QColor( 230, 230, 230, 100) );
	linearGrad.setColorAt(0, QColor( 250, 250, 250, 200 ) );
	painter->setBrush( QBrush( linearGrad ) );
	painter->drawRect( this->boundingRect() );

	painter->setBrush( Qt::NoBrush );
	painter->setPen( QPen( QBrush( QColor( 0, 0, 0, 100) ), 2 ) );
...
void NavigationBlock::mouseMoveEvent( QGraphicsSceneMouseEvent *event )
{
	if( _ONITEM > -1 )
	{
		...
		_bubbles[_ONITEM]->mySetPos( QPointF(_bubbles[_ONITEM]->pos().x() + offX * SCALE, _bubbles[_ONITEM]->pos().y() + offY * SCALE) );
		//this->_bubbles[_ONITEM]->mySetPos(  QPointF( _bubbles[_ONITEM]->pos().x() + offX * SCALE, _bubbles[_ONITEM]->pos().y() + offY * SCALE ));
		_lastPos = event->pos();
	}
	else if( _ONVIEWPORT )
	{
...
void NavigationBlock::wheelEvent( QGraphicsSceneWheelEvent *event )
{
	if( this->_rect.contains( QPoint( event->pos().x(), event->pos().y() ) ) )
	{
		...
		emit updateCoverage( _rectOffset );
	}
...
{
	this->_rect.setWidth( this->scene()->width()/20 );
...
void NavigationBlock::drawAllBubbles(QPainter *painter)
{
	...
	for( int i = 0; i < this->_bubbles.size(); i++ )
	{
		...
		painter->setBrush( QBrush( this->getQColorByIndex( this->_bubbles[i]->getType() ) ) );
