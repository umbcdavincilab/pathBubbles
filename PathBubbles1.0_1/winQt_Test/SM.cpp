...
QString SM::checkPathwayPath( QString name )
{

	QString result;
	//*************************************************
	QString camberPath = name + "1pathway.txt";
...
QList<int> SM::findHighlight( QRectF rect )
{
	QList<int> frameIndex;
	_INIT = false;
	highLightedList.clear();
	int selected = -1;
	int begin = TDHelper::Min( int(rect.topLeft().x()), int(rect.topRight().x()) );
	...
	if( this->_data[selected]->getType() == 1 || this->_data[selected]->getType() == 3 )
	{
		...
		maxHeight = TDHelper::Max( int(rect.y()), int(rect.y()) + int(rect.height()) );
		minHeight = TDHelper::Min( int(rect.y()), int(rect.y()) + int(rect.height()) );
		//************************************************
