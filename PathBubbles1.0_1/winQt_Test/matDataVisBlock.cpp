...
MatDataVisBlock::MatDataVisBlock( int size, int x, int y ) 
: PlotBase(size, x, y )
, codeLine("")
{
	...
	xDataType = ONEDTMAT;
	yDataType = ONEDTMAT;
	xIntervalNum = yIntervalNum = INTERNALNUM;
}
...
void MatDataVisBlock::plotXY( QPainter* painter )
{
	...
	xIntervalNum = m_size_width/30 > INTERNALNUM ? INTERNALNUM : m_size_width/30;
	yIntervalNum = m_size_height/30 > INTERNALNUM ? INTERNALNUM : m_size_height/30;
