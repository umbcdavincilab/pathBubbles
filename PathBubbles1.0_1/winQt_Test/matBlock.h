...
public:
	MatBlock( int x, int y );
	~MatBlock();

	void cloneItem( ItemBase* another );
	void updateCopyFun( QString tmpFunName, int copyNum );
	...
	bool getRunFlag(){return this->_RUNFLAG;};
	void setRunFlag(bool flag){this->_RUNFLAG=flag;};
...
signals:
	...
	void addParameterFromMat( QList<QString> pname, QString run_num, QList< QList<double> > dataResult );
...
private:
	void generateAppendCode( QString str, QList< QString > curPara, QString curfunName );
	...
	QList< QString > functionName;
	QList< QList< QString > > paraName;
	QList< QString > codes;
	...
	QString appendCode;
	int FunNumber;
	map< QString, int > FunHash;
	QList< QList<int> > FunCopyNum;
};
