...
{   	
	...
	//_scale=2.0;	
	//_note = new TextNote( this, _scene);
...
void PathBubble1::PathWayDataInitiation_1()//for generate treering without generating the hierarch pathway graph
{
	...
	vector<vector<QString>> _smallMoleculeName, _EmptyName, _DnaName, _ANodeName, _emptyName;  //start from 1

	_physicalEntityName.push_back(temp8);

	vector<QString> temp7;  
	temp7.push_back("");
	...
	if(toplevel&&_name.size()!=0)
	{
			...
			//tname = _name + "0expression.txt";  sname= tname.toStdString();  ename=sname.c_str();
			//_scene->pwdParser->readexpression("data/Reactome_Pathway_Data/TGF0expression.txt", _scene->_quantityName[m_pathwayID], _scene->_geneInfo[m_pathwayID], _scene->_quantity[m_pathwayID], _scene->_minmax[m_pathwayID]);
			//AVCRGBs=pwdParser->readMunshell();
			...
			//savePathWayforTreeRing(pathName, edge);

			//if(drawOrthology)
			//orthology = _scene->pwdParser->matchSymbol(_scene->orthologySymbol,  _proteinName);
			...
			ANodeNum=0;
			vector<QRectF> temp99(ANodeNum+1, rect);
			ANodePos=temp99;
					
			//getNameforConverted(_8convertedProtein, _8convertedComplex, _8convertedEntity, _scene->_complexNameD[dataID], _scene->_proteinNameD[dataID], _scene->_physicalEntityNameD[dataID]);
			...
			//QString pathName1 = _scene->pwdParser->getPathGraphHie(_pathName); 		
			//savePathWayforTreeRing(pathName1, edge);
			//_scene->pwdParser->TraceSteps( _pathwayStep, edge, reactionPos, _1pathway, _5Control, _Catalysis, _3pathwayStepCatalysis, _3pathwayStepReaction, _scene->_biochemicalReaction[m_pathwayID], _scene->drawSmallmolecule);
			...
			_protein.resize(proteinNum+1);
			_complex.resize(complexNum+1);	
			_entity.resize(physicalEntityNum+1);
			_ANode.resize(ANodeNum+1);
			_smallMolecule.resize(smallMoleculeNum+1);
			_Dna.resize(DnaNum+1);
			_reaction.resize(reactionNum+1);
			_compartment.resize(compartmentNum+1);
			_Empty.resize(EmptyNum+1);
			...
			/*
			if( _pathName=="Urea cycle" || _pathName=="Acetylcholine Binding And Downstream Events" || !pathWayRead(_whole, _VertexInfo, _Vertex, Rank0, Ranks,_newPos,_newCompartmentPos, _Edge) || !pathWayPoiRead(_whole, complexPos_0, proteinPos_0, smallMoleculePos_0, DnaPos_0, EmptyPos_0, reactionPos_0, physicalEntityPos_0, compartmentPos_0, ANodePos_0, CompartmentContain_0, _scale, dCenter, false)  )
			{
				...
				pathWayRecord(_whole, _VertexInfo, _Vertex, Rank0, Ranks,_newPos,_newCompartmentPos, _Edge);
				
            }
			//if(preComputed)
			
			//removeNotContainedinCompartment(CompartmentContain_0);
			...
			{	
				...
				putReactionToCompartment(_reaction, _scene->CompartmentContain[m_pathwayID], Ranks, _VertexInfo);	
			}				
	...
	{		
		...
		{
			lastStoredfileName = _scene->pwdParser->loadPathGraphPreparation(_pathName);
		}		
		//QString lastStoredfileName = QFileDialog::getOpenlastStoredfileName(0, tr("Open PathWay Stored as xml file"), pathName,".xml (*.xml)");//"c:\\file.x", "X files (*.x)"
		//lastStoredfileName = QFileDialog::getOpenlastStoredfileName(NULL , tr("Open PathWay"), "", tr("(*.xml)"));
		...
		if(!preComputed)
		{
			...
			pathWayPoiRecord(complexPos_0, proteinPos_0, smallMoleculePos_0, DnaPos_0, EmptyPos_0, reactionPos_0, physicalEntityPos_0, compartmentPos_0, ANodePos_0, CompartmentContain_0, _scale, dCenter, false);
			//pathWayPoiRead(_whole, complexPos_0, proteinPos_0, smallMoleculePos_0, DnaPos_0, EmptyPos_0, reactionPos_0, physicalEntityPos_0, compartmentPos_0, ANodePos_0, CompartmentContain_0, _scale, dCenter, false);

			pathWayPoiRecord(complexPos_1, proteinPos_1, smallMoleculePos_1, DnaPos_1, EmptyPos_1, reactionPos_1, physicalEntityPos_1, compartmentPos_1, ANodePos_1, CompartmentContain_1, _scale, dCenter, true);
		}
		aveFontSize=11;
		getGraphToBePaint();
	    //pathWayPoiRead(_whole, complexPos_1, proteinPos_1, smallMoleculePos, DnaPos_1, EmptyPos_1, reactionPos_1, physicalEntityPos_1, compartmentPos_1, ANodePos_1, CompartmentContain_1, _scale, dCenter, true);

		savePathWay(firstfileName, complexPos, proteinPos, smallMoleculePos, DnaPos, reactionPos, physicalEntityPos, ANodePos, edge);			
		_scene->backupItemPos(m_pathwayID, complexPos, proteinPos, smallMoleculePos, DnaPos, EmptyPos, reactionPos, physicalEntityPos, compartmentPos, ANodePos, _scale, dCenter, drawSmallmolecule);				
		*/		
...
void PathBubble1::PathWayDataInitiation()
{
	currentPathwayGraphit=pathwayGraphStored.end();
	currentHighlightedit=highlightedStored.end();
	currentSharedit=sharedStored.end();
	...
	if(toplevel&&_name.size()!=0)
	{
		...
		if( _pathName=="Urea cycle" || _pathName=="Acetylcholine Binding And Downstream Events" || _pathName== "Acetylation" || !pathWayRead(_whole, _VertexInfo, _Vertex, Rank0, Ranks,_newPos,_newCompartmentPos, _Edge) || !pathWayPoiRead(_whole, complexPos_0, proteinPos_0, smallMoleculePos_0, DnaPos_0, EmptyPos_0, reactionPos_0, physicalEntityPos_0, compartmentPos_0, ANodePos_0, CompartmentContain_0, _scale, dCenter, false)  )
		{
			preLocateNodes(complexPos,proteinPos,smallMoleculePos);
	...
	{		
		...
		if(!preComputed)
		{
			...
			pathWayPoiRecord(complexPos_0, proteinPos_0, smallMoleculePos_0, DnaPos_0, EmptyPos_0, reactionPos_0, physicalEntityPos_0, compartmentPos_0, ANodePos_0, CompartmentContain_0, _scale, dCenter, false);
			//pathWayPoiRead(_whole, complexPos_0, proteinPos_0, smallMoleculePos_0, DnaPos_0, EmptyPos_0, reactionPos_0, physicalEntityPos_0, compartmentPos_0, ANodePos_0, CompartmentContain_0, _scale, dCenter, false);
			pathWayPoiRecord(complexPos_1, proteinPos_1, smallMoleculePos_1, DnaPos_1, EmptyPos_1, reactionPos_1, physicalEntityPos_1, compartmentPos_1, ANodePos_1, CompartmentContain_1, _scale, dCenter, true); //_scale may not work for graph with small moleculer
		}
		if(drawSmallmolecule)
		{
			_scale=1.3; //work for 
...
void PathBubble1::updateNodeNums(vector<vector<int>> edge, int &proteinNum, int &complexNum, int &physicalEntityNum, int &smallMoleculeNum, int &DnaNum,
...
{	
	...
	addANode('P', Pmax, proteinNum, _proteinName, _compartmentName, CompartmentContain, proteinPos);//, vector<vector<QRectF>> proteinPos, vector<vector<QRectF>> complexPos, vector<vector<QRectF>> physicalEntityPos, vector<vector<QRectF>> smallMoleculePos, vector<vector<QRectF>> DnaPos, vector<vector<QRectF>> EmptyPos);
	addANode('C', Cmax, complexNum, _complexName, _compartmentName, CompartmentContain,complexPos);
	addANode('E', Emax, physicalEntityNum, _physicalEntityName, _compartmentName, CompartmentContain,  physicalEntityPos);
	addANode('S', Smax, smallMoleculeNum, _smallMoleculeName, _compartmentName, CompartmentContain, smallMoleculePos);
	addANode('D', Dmax, DnaNum, _DnaName, _compartmentName, CompartmentContain, DnaPos);
	addANode('N', Nmax, emptyNum, _emptyName, _compartmentName, CompartmentContain, EmptyPos);

	/*int type;
	type='P';
	if(Pmax>=proteinNum)
	{
		_proteinName.resize(Pmax+1);
	}
	for(int i=proteinNum; i<Pmax+1; i++)
	{
		vector<int> item;
		...
		_proteinName[i].push_back(""); //name	
		_proteinName[i].push_back("");
		_proteinName[i].push_back("");
		_proteinName[i].push_back("");
		_proteinName[i].push_back("");
	}
...
void PathBubble1::addANode(int type, int newNum, int &nodeNum, vector<vector<QString>> &nodeName, vector<vector<QString>> _compartmentName, vector<set<vector<int>>> &CompartmentContain, vector<QRectF> &nodePos)
{
	//type='P';
	if(newNum>=nodeNum)
	{
		nodeName.resize(newNum+1);
	}
	for(int i=nodeNum+1; i<newNum+1; i++)
	{
		vector<int> item;
		item.push_back(type); item.push_back(i);
		set<vector<int>> empty;
	...
	int oriSize=nodePos.size();
	nodePos.resize(nodeNum+1);

	for(int i=oriSize; i<newNum+1; i++)
	{
...
{//resize: true, remove the compartment if it is empty
	...
	for(int i=0; i<totalNum && i<_VertexInfo.size(); i++)
	{
					...
					{		
						int id=Ranks[i][k][l];
						 if(id<_newPos.size() && _newPos[id].x()>-999.99)					 
						 {
							...
							//VertexInfo[Rank[i][j]][3]=roomID;		
							//if( (_VertexInfo[id][0]=='S'||_VertexInfo[id][0]=='D') ) //VertexInfo[Rank[i][j]][3]==-1 
							if( i<cContain.size() )
							{
...
bool PathBubble1::pathWayPoiRead(bool whole, vector<QRectF> &complexPos, vector<QRectF> &proteinPos, vector<QRectF> &smallMoleculePos, vector<QRectF> &DnaPos, vector<QRectF> &EmptyPos, vector<QRectF> &reactionPos, vector<QRectF> &physicalEntityPos, vector<QRectF> &compartmentPos, vector<QRectF> &ANodePos, vector<set<vector<int>>> &CompartmentContain, float &_scale, QPointF &dCenter, bool drawSmallmolecule)
{//resize: true, remove the compartment if it is empty
 	//int totalNum = complexNum + proteinNum + physicalEntityNum + smallMoleculeNum + DnaNum + reactionNum + 2;
...
void PathBubble1::pathWayRecord(bool whole, vector<vector<int>> _VertexInfo, vector<vector<vector<int>>> _Vertex, vector<vector<int>> Rank0, vector<vector<vector<int>>> Ranks, vector<QRectF> _newPos, vector<QRectF> _newCompartmentPos, vector<vector<int>> _Edge, bool drawSmallmolecule)
{//resize: true, remove the compartment if it is empty
 	int totalNum = complexNum + proteinNum + physicalEntityNum + smallMoleculeNum + DnaNum + reactionNum + 2;
	QString pathName = _scene->pwdParser->getPathGraphHie(_pathName); //_scene->pwdParser->savePathGraphPreparation(_pathName);		
	if(drawSmallmolecule)
	...
	out << "_Vertex.size()" << " " <<  _Vertex.size() << "\n";	
	for(int i=0; i<_Vertex.size(); i++)
	{	
			out << "_Vertex[i].size()" << " " <<  _Vertex[i].size() << "\n";				
			for(int k=0; k<_Vertex[i].size(); k++) 
			{
				out << "_Vertex[i][k].size()" << " " <<  _Vertex[i][k].size() << "\n";		
				for(int l=0; l<_Vertex[i][k].size(); l++) 
				{		
					out << " " << _Vertex[i][k][l] ;						
				}	
				out << "\n";
	...
	out << "_newPos.size()" << " " <<  _newPos.size() << "\n";	
	for(int i=0; i<_newPos.size(); i++)
	{
		out << " " <<_newPos[i].x();			
		out << " " <<_newPos[i].y();			
		out << " " <<_newPos[i].width();			
		out << " " <<_newPos[i].height();			
		out << "\n";
	}

	out << "_newCompartmentPos.size()" << " " <<  _newCompartmentPos.size() << "\n";	
	for(int i=0; i<_newCompartmentPos.size(); i++)
	{
		out << " " <<_newCompartmentPos[i].x();			
		out << " " <<_newCompartmentPos[i].y();			
		out << " " <<_newCompartmentPos[i].width();			
		out << " " <<_newCompartmentPos[i].height();			
		out << "\n";
	}

	

	out << "_Edge.size()" << " " <<  _Edge.size() << "\n";	
	...
	{       
        out << "Rank0.size()" << " " <<  Rank0.size() << "\n";	
		for(int i=0; i<Rank0.size(); i++)
		{	
			vector<int> Rank0_1;
			out << "Rank0[i].size()" << " " <<  Rank0[i].size() << "\n";	
			for(int j=0; j<Rank0[i].size(); j++) 
			{
				out << " " <<Rank0[i][j] ;	
			}	
		...
		out << "Ranks.size()" << " " <<  Ranks.size() << "\n";	
		for(int i=0; i<Ranks.size(); i++)
		{	
			    out << "Ranks[i].size()" << " " <<  Ranks[i].size() << "\n";				
				for(int k=0; k<Ranks[i].size(); k++) 
				{
					out << "Ranks[i][k].size()" << " " <<  Ranks[i][k].size() << "\n";		
					for(int l=0; l<Ranks[i][k].size(); l++) 
					{		
						out << " " << Ranks[i][k][l] ;						
					}	
...
bool PathBubble1::pathWayRead(bool whole, vector<vector<int>> &_VertexInfo, vector<vector<vector<int>>> &_Vertex, vector<vector<int>> &Rank0, vector<vector<vector<int>>> &Ranks, vector<QRectF> &_newPos, vector<QRectF> &_newCompartmentPos, vector<vector<int>> &_Edge, bool drawSmallmolecule)
{//resize: true, remove the compartment if it is empty
 	//int totalNum = complexNum + proteinNum + physicalEntityNum + smallMoleculeNum + DnaNum + reactionNum + 2;
	vector<vector<int>> VertexInfo_1;
	vector<vector<vector<int>>> Vertex_1;
	...
	{			   
		QString line = in.readLine();    
		if(!line.contains("_VertexInfo.size()"))
			return false;
		...
		for(int i=0; i<I; i++)
		{
			...
			for(int j=1; j<sl.size(); j++)
			{
				VertexInfo_1[i].push_back(sl[j].toInt());
			}
		}


		line = in.readLine();    
		if(!line.contains("_Vertex.size()"))
			return false;
		...
		{	
			line = in.readLine();    
			if(!line.contains("_Vertex[i].size()"))
				return false;
			QStringList sl=line.split(' ');	
			int K=sl[1].toInt();
			Vertex_1[i].resize(K);  
			...
			for(int k=0; k<K; k++) 
			{
				...
				{		
					Vertex_1[i][k].push_back(s2[l].toInt());
					//out << " " << Ranks[i][k][l];						
				}	
				//out << "\n";
		 	}
		...
		for(int i=0; i<I; i++)
		{
			...
			QStringList sl=line.split(' ');	
			float x,y,w,h;
			x=sl[1].toFloat(); y=sl[2].toFloat(); w=sl[3].toFloat(); h=sl[4].toFloat();
			newPos_1[i]=QRectF(x,y,w,h);
		}
		
		line = in.readLine();    
		if(!line.contains("_newCompartmentPos.size()"))
			return false;
		...
		for(int i=0; i<I; i++)
		{
			...
			float x,y,w,h;
			x=sl[1].toFloat(); y=sl[2].toFloat(); w=sl[3].toFloat(); h=sl[4].toFloat();
			newCompartmentPos_1[i]=QRectF(x,y,w,h);
		}


	    line = in.readLine();    
		if(!line.contains("_Edge.size()"))
			return false;
		...
		{	
			...
			int J=sname[1].toInt();
			line = in.readLine();   
			QStringList sl1=line.split(' ');	
			for(int j=1; j<sl1.size(); j++) 
			{		
				Rank0_1[i].push_back(sl1[j].toInt());
				//out << " " <<Rank0[i][j] ;	
			}	
			//out << "\n";
		}	
		...
		{	
			...
			for(int k=0; k<K; k++) 
			{
				...
				for(int l=1; l<s2.size(); l++) 
				{		
					Ranks_1[i][k].push_back(s2[l].toInt());
					//out << " " << Ranks[i][k][l];						
				}	
				//out << "\n";
		 	}
	...
	_newPos = newPos_1;
	_Edge = Edge_1;
	_newCompartmentPos = newCompartmentPos_1;
	_VertexInfo=VertexInfo_1;
	Rank0=Rank0_1;
	Ranks=Ranks_1;	
	file.close(); 
...
void PathBubble1::HierarchicalTreeNodeLocation(bool whole, vector<vector<int>> _VertexInfo, vector<vector<int>> Rank0, vector<vector<vector<int>>> Ranks)
{
	...
	//for(int i=0; i<totalNum && i<newpos.size(); i++)
	for(int i=0; i< _VertexInfo.size(); i++)
	{		
		int id=_VertexInfo[i][1];
...
void PathBubble1::openPreNodeNotes()
{
	...
	for(int i=0; i<Type.size(); i++)
	{
		...
		if(flag)
		{
			...
			switch(type)
			{
				case 'C': if(id>=complexRect.size())  return;
							  pos=complexRect[id].center();  break;
				case 'E': if(id>=physicalEntityRect.size())  return;
					           pos=physicalEntityRect[id].center();   break;				
				case 'S': if(id>=smallMoleculeRect.size())  return;
					      pos=smallMoleculeRect[id].center();   break;							
				case 'P':  if(id>=proteinRect.size())  return;
					             pos=proteinRect[id].center();   break;			 
				case 'D': if(id>=DnaRect.size())  return;
					         pos=DnaRect[id].center();   break;		
				case 'N': if(id>=EmptyRect.size())  return;
					         pos=EmptyRect[id].center();   break;
				case 'R':  if(id>=reactionRect.size())  return;
					          pos=reactionRect[id].center();   break;
			}	
		    _scene->openNodeNote(m_pathwayID, type, id, this, pos + this->scenePos(), text, Cid);		
		}
	}
			...
			if(toHighlight)
		    {
				...
				for(int i=0; i<mlist.size(); i++) 
				{
					...
					for(set<int>::iterator it=sharedNode[pbubble->m_pathwayID].begin(); it!=sharedNode[pbubble->m_pathwayID].end(); it++)
					{
						...
						if(m==8 || nodePos[pbubble->m_pathwayID][id].x()>-990.0)
...
void PathBubble1::sharedDifferedLinkedExpressedHighLightedSizeCheck()
{
	...
	/*for(deque< vector <set < vector <int> > > >::iterator it=headPathway -> linkedStoredinHead.begin(); it!=headPathway -> linkedStoredinHead.end(); it++)
	{
		headPathway -> linkedStoredinHead.resize(fsize);
	}*/
...
vector < set < vector <int> > > PathBubble1::assignSharedSets()
{
	...
	{		
		if(sharedSets[j].size()>0 && sharedSets[i].size()>0)
		{
			...
			{
				//setAItemColored(node1[0], 'P', j, GO_UP);			
...
void PathBubble1::storeHighlighted()
{	 	 
...
void PathBubble1::storeDiffered()
{	
...
void PathBubble1::storeExpressed()
{	
...
{	
	...
	/*PathBubble1* pbubble = _scene->_pathBubbles[pid];		 								 		   		 
	vector<int> item;
	deque< set < vector <int> > >::iterator it=pbubble->linkedStored.begin();
	item.push_back(pid); item.push_back(type);  item.push_back(id);
    
	if(it==pbubble->linkedStored.end())
		return -1;
	else
	{
		if(headPathway==NULL)
		...
		if(headPathway->linkedStoredinHead.size()==0)
			   return -1;

		deque< vector < set < vector <int> > > > ::iterator it= headPathway->linkedStoredinHead.begin();
		...
		int count=0;				
		while(it!=headPathway->currentLinkedinHeadit)
		{	    
			it++;
			count++;
			linked=*it;	
...
/*
	//link in Headbubble, add it to help color along the linked edge
	if(headPathway->currentLinkedinHeadit!=headPathway->linkedStoredinHead.end())
	{
		vector <set < vector <int> > > linked;
		if(headPathway->currentLinkedinHeadit!=headPathway->linkedStoredinHead.begin())
		{
			headPathway->currentLinkedinHeadit--;
			linked=*headPathway->currentLinkedinHeadit;
		}			 
		for(int k=0; k<linked.size();k++)
		{
			PathBubble1* pBubble = _scene->_pathBubbles[k];		
			if(_scene->_pathBubbles[k])
		    if(_scene->_pathBubbles[k]!=NULL)
			for(set<vector<int>>::iterator it=linked[k].begin(); it!=linked[k].end(); it++)
			{
				vector<int> temp=*it,temp0(3,0),temp1(3,0);
				int pid, type, id;
				if(temp.size()>3)
				...
				switch(type)
				{
					case 'P': pBubble->linkedProtein[pid].insert(id);          break;
					case 'S': pBubble->linkedSmallMolecule[pid].insert(id);    break; 
					case 'C': pBubble->linkedComplex[pid].insert(id);          break;
					case 'D': pBubble->linkedDna[pid].insert(id);              break;
					case 'R': pBubble->linkedReaction[pid].insert(id);         break;
					case 'E': pBubble->linkedPhysicalEntity[pid].insert(id);   break;					
					case 'M': pBubble->linkedCompartment[pid].insert(id);      break;	
					case 'W': pBubble->linkedPathway[pid].insert(id);          break;	
					case 'A': 	
						temp1[0]=temp[4], temp1[1]=temp[5], temp1[2]=temp[6];
						temp0[0]=temp[1], temp0[1]=temp[2], temp0[2]=temp[3];
						edge.push_back(temp0);
						edge.push_back(temp1);
						pBubble->linkedEdge[pid].insert(edge);
						break;	
				}	
			}
		}
...
///////// linked infor stored in the headbubble
/*if(headPathway->currentLinkedinHeadit!=headPathway->linkedStoredinHead.end())
	{
		vector <set < vector <int> > > linked;
	if(headPathway->currentLinkedinHeadit!=headPathway->linkedStoredinHead.end())
...
void PathBubble1::sharedDifferedLinkedExpressedtoHighlighted(bool toHighlight)
{ //toHighLight: false is only for expression search
	//if nothing is highlighted
	//highlight pathway
	sharedDifferedLinkedExpressedHighLightedSizeCheck();
...
void PathBubble1::searchLinkedNode( int pflag, int cflag, int wflag, bool Andflag)
{       
	    ...
	    //for(int m=0;m<7;m++)
		{
			...
			for(set<vector<int>>::iterator it=sID.begin(); it!=sID.end(); it++)
			{
				...
				//search node
				{
					...
					switch(type)
					{
						...
						case 'M': 
							...
							{   
								...
								switch(temp[0])
								{
									case 'P': edgeRecord2.push_back(pbubble->_protein[temp[1]]);       break;
									case 'L': edgeRecord2.push_back(pbubble->_ANode[temp[1]]);       break;
									case 'S': edgeRecord2.push_back(pbubble->_smallMolecule[temp[1]]); break;
									case 'C': edgeRecord2.push_back(pbubble->_complex[temp[1]]);       break;
									case 'D': edgeRecord2.push_back(pbubble->_Dna[temp[1]]);           break;
									case 'R': edgeRecord2.push_back(pbubble->_reaction[temp[1]]);      break;
									case 'E': edgeRecord2.push_back(pbubble->_entity[temp[1]]);        break;							    
								}							
							}
							...
							for(int l=0; l<edgeRecord2[k].size(); l++)
							{
								...
								temp2[0]=m_pathwayID; temp2[1]=node[0]; temp2[2]=node[1];		
								if (temp2[1]=='R' && temp2[2]>=0 && _scene->_reactionPos[pbubble->m_pathwayID][temp2[2]].x()>-999.99)//keqin //(
								{
									edgeRecord3=pbubble->_reaction[temp2[2]];								
								}								
								edgeRecord3.push_back(temp2);
								//if (temp2[2]>=0 && _scene->_reactionPos[pbubble->m_pathwayID][temp2[2]].x()>-999.99)//keqin //(temp2[1]=='R'&&
								{
									//edgeRecord3=pbubble->_reaction[temp2[2]];								
									for(int l=0; l<edgeRecord3.size(); l++)
									{
										node=edgeRecord3[l];

										temp[0]=m_pathwayID; temp[1]=node[0]; temp[2]=node[1];																		
										
										edge1[0]=temp, edge1[1]=temp2;				
										edge2[1]=temp, edge2[0]=temp2;	

										vector<vector<int>> sEdge=findSpannedEdge(scene->edgeconnections, temp2);
										bool flag= (updatedEdge_1.find(edge1)!=updatedEdge_1.end()||updatedEdge_1.find(edge2)!=updatedEdge_1.end());
					...
					vector<int> node,temp(3,0);
					temp2[0]=m_pathwayID; temp2[1]=type; temp2[2]=id;
					for(int k=0; k<edgeRecord.size(); k++)
					{
						node=edgeRecord[k];
						...
						vector<vector<int>> sEdge=findSpannedEdge(scene->edgeconnections, temp2);
						bool flag= (updatedEdge_1.find(edge1)!=updatedEdge_1.end()||updatedEdge_1.find(edge2)!=updatedEdge_1.end()) || (updatedEdge_2.find(edge1)!=updatedEdge_2.end()||updatedEdge_2.find(edge2)!=updatedEdge_2.end()) || (updatedEdge_3.find(edge1)!=updatedEdge_3.end()||updatedEdge_3.find(edge2)!=updatedEdge_3.end());
					...
					for(int m=0; m<edgeRecord.size(); m++)
					{
						node=edgeRecord[m];
						temp[0]=m_pathwayID; temp[1]=node[0]; temp[2]=node[1];
						for(int k=0; k<edgeRecord1.size(); k++)
						for(int l=0; l<edgeRecord1[k].size(); l++)
						{
							node=edgeRecord1[k][l];
							temp2[0]=m_pathwayID; temp2[1]=node[0]; temp2[2]=node[1];
							edge1[0]=temp, edge1[1]=temp2;				
							edge2[1]=temp, edge2[0]=temp2;	
							vector<vector<int>> sEdge=findSpannedEdge(scene->edgeconnections, temp2);
				...
				else if(!pflag && cflag==1 && !wflag) //for compartment
				{
...
bool PathBubble1::isSameNode(int pid1, int type1, int id1, int pid2, int type2, int id2)
{
	...
	switch(type1)
	{
		...
		case 'R': return(isSameReaction(pid1, id1, pid2, id2)); 
			...
			{	
				...
				if(type!='W'&& type!='M') //node to node
				{
					...
					switch(type)
			        {
						...
						case 'E': iName=_scene->_physicalEntityNameD[Did][id];      break;
						case 'L': iName=_scene->_ANodeName[Pid][id];      break;
						case 'P': iName=_scene->_proteinNameD[Pid][id];      break;
						case 'S': iName=_scene->_smallMoleculeNameD[Did][id];      break;
						case 'R': iName=_scene->_reactionNameD[Did][id];      break;
					}
					/*nodePos[m_pathwayID][j];
					int s1=nodeName[Did].size();
					nodeName[Did][j];
					int s2=nodeName[Did][j].size();
					nodeName[Did][j][0];
					isSameReaction(Pid, id, m_pathwayID, j);*/
				    if( nodePos[m_pathwayID][j].x()>-999.99 && ( (type!='R' && iName == nodeName[m_pathwayID][j]) || (type=='R' && iName[1] == nodeName[m_pathwayID][j][1]) ) )// && isSameReaction(Pid, id, m_pathwayID, j)) ) //node level 
				    {
                       if(pid==-1||pid==m_pathwayID) //pathway level
					   {
						   if(cid!=-1)
		...
		{			
			...
			{	
			    ...
			    else if(type=='M') //compartment to compartment			    
				{
					vector<QString> iName;
					iName=_scene->_compartmentName[Pid][cid];  
					if(j<nodePos[m_pathwayID].size() && nodePos[m_pathwayID][j].x()>-999.99) //node level 
				    {
                       if(pid==-1||pid==m_pathwayID) //pathway level
					   {
						   if((cid==-1||cid==j)&&nodePos[m_pathwayID][j].x()>-999.99)//cid compartment level
				...
				{										
					if(pid==m_pathwayID) //pathway level
					{
						   if(nodePos[m_pathwayID][j].x()>-999.99)//cid compartment level
				           { 
							    for(int i=0; i<count.size(); i++)
							  {					
									if(count[i][0]==Pid && count[i][1]==pid && count[i][2]==cid)
...
{ //some protein, complex appear in the reaction but can not find its compartment (or any other information event its name)
	...
	for(int i=1; i<_complexName.size(); i++)
	 {
		 bool flag=true;
	...
	for(int i=1; i<_proteinName.size(); i++)
	 {
		 bool flag=true;
		 for(int j=0;j<CompartmentContain.size();j++)
		 {
	...
	for(int i=1; i<_physicalEntityName.size(); i++)
	 {
		 bool flag=true;
		 for(int j=0;j<CompartmentContain.size();j++)
		 {
...
{  
	...
	{		
         ...
         if(end[0]!='R' || drawSmallmolecule) //drawSmallmolecule		 
	     {
...
bool PathBubble1::PathtoRank(vector<vector<vector<int>>> Vertex, vector<vector<int>> &VertexInfo, vector<vector<int>> Edge, vector<QRectF> &newPos, vector<set<vector<int>>> &CompartmentContain, vector<vector<int>> &Rank, vector<int> &path, int &mini, int roomID)
{
...
{  //Vertex: record edges
	...
	if(!PathtoRank(Vertex, VertexInfo, Edge, newPos, CompartmentContain, Rank, path, mini, roomID))
		return tempR;
	...
	/*if(roomID==8 && toplevel && this->_curFileName.contains("BMP"))
	{
		Rank[0].push_back(17); newPos[17].setY((float)VertexInfo[path[0]][2]/(path.size()-mini)*0.95+0.1);
	}*/
...
{//replace the complex with proteins contained according to _complexContain
	...
	int size;
	for(int i=1; i<_scene->_complexContain[m_pathwayID].size(); i++)
	{
...
/*int PathBubble1::read7protein(const char *name, vector<vector<QString>> &_proteinName)
{
	char ch[255], ch2[255];	
	...
	{	 
		...
		vector<string> buffer;
		  ...
		  for(int i=0; i<buffer.size(); i++)
		  {
			...
			for(int i=0;i<_scene->_compartmentName[m_pathwayID].size();i++)
			{
			   if(_scene->_compartmentName[m_pathwayID][i][0]==n3.c_str())
			   {
				   vector<int> item;
			...
			_proteinName[pid][0]=buffer[1].c_str();
			_proteinName[pid][1]=buffer[2].c_str();
			if(pid>max)
			    max=pid;
		...
		vector<string> buffer;
		  ...
		  c=buffer[0][0];
			...
			if(c=='C')
		    {
				...
				for(int i=0;i<_scene->_compartmentName[m_pathwayID].size();i++)
					{
						if(_scene->_compartmentName[m_pathwayID][i][0]==n1.c_str())
						{
							vector<int> item;
...
void PathBubble1::read9smallEntity(const char *name, vector<vector<QString>> & _smallMoleculeName, vector<vector<QString>> &_DnaName)
{
char ch[255], ch2[255];	
char c;
char e;	
	...
	do
	{
		...
		vector<string> buffer;
		  ...
		  n3=buffer[4];
			...
			if(c=='D')
		    {
				_DnaName[pid][0]=n1.c_str();
				_DnaName[pid][1]=n2.c_str();
		    }
		    else if(c=='S')
			{
				 _smallMoleculeName[pid][0]=n1.c_str();
				 _smallMoleculeName[pid][1]=n2.c_str();
			}	
...
{	
	...
	PathBubbleDelectionUpdate_1(m_pathwayID);
    logRemove();	
...
void PathBubble1::scaleNodeToBePaint(QPointF dCenter, float scale)
{
	...
	for(int i=0; i<ANodePos.size(); i++)
	{
		...
		{
			//center = fontRect[ocount].center();
			ANodeRect[i]=getNewNodeRect(ANodePos[i], 'E', dCenter, scale);			
			//fontSize[ocount]=;	
			getNodeFont(fontSize[ocount], fontRect[ocount], ANodeRect[i], _scene->_ANodeName[m_pathwayID][i][0], 'E', width, height, fixedSize, scale);
			avaSize = avaSize + fontSize[ocount];
			if(fontSize[ocount]>maxFontSize) 
				maxFontSize=fontSize[ocount];
	        sizeNum++;
...
{	
	...
	{		   
	   ...
	   switch(type)
	   {
			case 'M': if(compartmentRect[id].width()>0) 
					  {
						  compartmentRect[id] = getCompartmentToBePaint(boundaryWidth[ID], fontSize[ID], insideColor[ID], boundaryColor[ID], fontColor[ID], fontRect[ID], compartmentPos, _scene->_compartmentName[m_pathwayID][id][0], id, _scene->getGroup(this)->sharedSets, _scene->getGroup(this)->expressedSets,fixedSize, dCenter, _scale);
						  fontSize[ID]=maxFontSize;
					  }
	                  break;
			case 'C': if(complexRect[id].width()>0) 
					      complexRect[id] = getNodetoBePaint(boundaryWidth[ID], fontSize[ID], insideColor[ID], boundaryColor[ID], fontColor[ID], fontRect[ID], complexPos[id], _scene->_complexNameD[dataID][id][0], 'C', id, _scene->getGroup(this)->sharedSets, _scene->getGroup(this)->expressedSets,fixedSize, dCenter, _scale);	        
				      break; 
			case 'R': if(reactionRect[id].width()>0) 
						reactionRect[id] = getNodetoBePaint( boundaryWidth[ID], fontSize[ID], insideColor[ID], boundaryColor[ID], fontColor[ID], fontRect[ID], reactionPos[id], _scene->_reactionNameD[dataID][id][1], 'R', id, _scene->getGroup(this)->sharedSets, _scene->getGroup(this)->expressedSets,fixedSize, dCenter, _scale, _scene->_reactionNameD[dataID][id][0]);			
				      break;
			case 'S': if(smallMoleculeRect[id].width()>0) 
						  smallMoleculeRect[id]=getNodetoBePaint( boundaryWidth[ID], fontSize[ID], insideColor[ID], boundaryColor[ID], fontColor[ID], fontRect[ID], smallMoleculePos[id], _scene->_smallMoleculeNameD[dataID][id][0], 'S', id, _scene->getGroup(this)->sharedSets, _scene->getGroup(this)->expressedSets,fixedSize, dCenter, _scale);			
					  break;
			case 'D': if(DnaRect[id].width()>0) 
						  DnaRect[id] = getNodetoBePaint(boundaryWidth[ID], fontSize[ID], insideColor[ID], boundaryColor[ID], fontColor[ID], fontRect[ID], DnaPos[id], _scene->_DnaNameD[dataID][id][0], 'D', id, _scene->getGroup(this)->sharedSets, _scene->getGroup(this)->expressedSets,fixedSize, dCenter, _scale);							
				      break;	
			case 'E': if(physicalEntityRect[id].width()>0) 
						  physicalEntityRect[id]=getNodetoBePaint( boundaryWidth[ID], fontSize[ID], insideColor[ID], boundaryColor[ID], fontColor[ID], fontRect[ID], physicalEntityPos[id], _scene->_physicalEntityNameD[dataID][id][0], 'E', id, _scene->getGroup(this)->sharedSets, _scene->getGroup(this)->expressedSets, fixedSize, dCenter, _scale);		  
				      break;
			case 'L': if(ANodeRect[id].width()>0) 
						  ANodeRect[id]=getNodetoBePaint( boundaryWidth[ID], fontSize[ID], insideColor[ID], boundaryColor[ID], fontColor[ID], fontRect[ID], ANodePos[id], _scene->_ANodeName[m_pathwayID][id][0], 'L', id, _scene->getGroup(this)->sharedSets, _scene->getGroup(this)->expressedSets, fixedSize, dCenter, _scale);		  
				      break;
			case 'P': if(proteinRect[id].width()>0) 
						  proteinRect[id] = getNodetoBePaint(boundaryWidth[ID], fontSize[ID], insideColor[ID], boundaryColor[ID], fontColor[ID], fontRect[ID], proteinPos[id], _scene->_proteinNameD[dataID][id][0], 'P', id, _scene->getGroup(this)->sharedSets, _scene->getGroup(this)->expressedSets,fixedSize, dCenter, _scale);	 		  
				      break;
		}
	}
	for(int i=0; i<currentItemSelected.size(); i=i+2)
...
{	
	...
	{		   
	   ...
	   if(pid==m_pathwayID)
	   {
		   ...
		   switch(type)
			{
				case 'M': compartmentRect[id] = getCompartmentToBePaint(boundaryWidth[ID], fontSize[ID], insideColor[ID], boundaryColor[ID], fontColor[ID], fontRect[ID], compartmentPos, _scene->_compartmentName[m_pathwayID][id][0], id, _scene->getGroup(this)->sharedSets, _scene->getGroup(this)->expressedSets, fixedSize, dCenter, _scale);
						  fontSize[ID]=maxFontSize;
						  break;
				case 'C': complexRect[id] = getNodetoBePaint(boundaryWidth[ID], fontSize[ID], insideColor[ID], boundaryColor[ID], fontColor[ID], fontRect[ID], complexPos[id], _scene->_complexNameD[dataID][id][0], 'C', id, _scene->getGroup(this)->sharedSets, _scene->getGroup(this)->expressedSets, fixedSize, dCenter, _scale);	        
						  break; 
				case 'R': reactionRect[id] = getNodetoBePaint( boundaryWidth[ID], fontSize[ID], insideColor[ID], boundaryColor[ID], fontColor[ID], fontRect[ID], reactionPos[id], _scene->_reactionNameD[dataID][id][1], 'R', id, _scene->getGroup(this)->sharedSets, _scene->getGroup(this)->expressedSets, fixedSize, dCenter, _scale, _scene->_reactionNameD[dataID][id][0]);			
						  break;
				case 'S': smallMoleculeRect[id]=getNodetoBePaint( boundaryWidth[ID], fontSize[ID], insideColor[ID], boundaryColor[ID], fontColor[ID], fontRect[ID], smallMoleculePos[id], _scene->_smallMoleculeNameD[dataID][id][0], 'S', id, _scene->getGroup(this)->sharedSets, _scene->getGroup(this)->expressedSets, fixedSize, dCenter, _scale);			
						  break;
				case 'D': DnaRect[id] = getNodetoBePaint(boundaryWidth[ID], fontSize[ID], insideColor[ID], boundaryColor[ID], fontColor[ID], fontRect[ID], DnaPos[id], _scene->_DnaNameD[dataID][id][0], 'D', id, _scene->getGroup(this)->sharedSets, _scene->getGroup(this)->expressedSets, fixedSize, dCenter, _scale);							
						  break;	
				case 'E': physicalEntityRect[id]=getNodetoBePaint( boundaryWidth[ID], fontSize[ID], insideColor[ID], boundaryColor[ID], fontColor[ID], fontRect[ID], physicalEntityPos[id], _scene->_physicalEntityNameD[dataID][id][0], 'E', id, _scene->getGroup(this)->sharedSets, _scene->getGroup(this)->expressedSets, fixedSize, dCenter, _scale);		  
						  break;
				case 'L': 
					      ANodeRect[id]=getNodetoBePaint( boundaryWidth[ID], fontSize[ID], insideColor[ID], boundaryColor[ID], fontColor[ID], fontRect[ID], ANodePos[id], _scene->_ANodeName[m_pathwayID][id][0], 'L', id, _scene->getGroup(this)->sharedSets, _scene->getGroup(this)->expressedSets, fixedSize, dCenter, _scale);		  
						  break;
				case 'P': proteinRect[id] = getNodetoBePaint(boundaryWidth[ID], fontSize[ID], insideColor[ID], boundaryColor[ID], fontColor[ID], fontRect[ID], proteinPos[id], _scene->_proteinNameD[dataID][id][0], 'P', id, _scene->getGroup(this)->sharedSets, _scene->getGroup(this)->expressedSets, fixedSize, dCenter, _scale);	 		  
						  break;
			}
	   }
	}
	...
	{		   
	   ...
	   {		
		    ...
		    switch(type)
			{
				...
				case 'E': physicalEntityRect[id]=getNodetoBePaint( boundaryWidth[ID], fontSize[ID], insideColor[ID], boundaryColor[ID], fontColor[ID], fontRect[ID], physicalEntityPos[id], _scene->_physicalEntityNameD[dataID][id][0], 'E', id, _scene->getGroup(this)->sharedSets, _scene->getGroup(this)->expressedSets, fixedSize, dCenter, _scale);		  
						  break;
				case 'L': ANodeRect[id]=getNodetoBePaint( boundaryWidth[ID], fontSize[ID], insideColor[ID], boundaryColor[ID], fontColor[ID], fontRect[ID], ANodePos[id], _scene->_ANodeName[m_pathwayID][id][0], 'L', id, _scene->getGroup(this)->sharedSets, _scene->getGroup(this)->expressedSets, fixedSize, dCenter, _scale);		  
						  break;
				case 'P': proteinRect[id] = getNodetoBePaint(boundaryWidth[ID], fontSize[ID], insideColor[ID], boundaryColor[ID], fontColor[ID], fontRect[ID], proteinPos[id], _scene->_proteinNameD[dataID][id][0], 'P', id, _scene->getGroup(this)->sharedSets, _scene->getGroup(this)->expressedSets, fixedSize, dCenter, _scale);	 		  
						  break;
			}
...
//should return the ratio
	...
	if(!_whole)
	{
		for(int i=0; i<compartmentPos.size()-1; i++) //size?
		{
			...
			if(i < compartmentPos.size() && compartmentPos[i].x()>-999.99 && _scene->_compartmentName[m_pathwayID][i][0]!="whole")
			{
				tRect= getCompartmentToBePaint( _boundaryWidth, _fontSize, _insideColor, _boundaryColor, _fontColor, _fontRect, compartmentPos, _scene->_compartmentName[m_pathwayID][i][0], i, sharedSets, expressedSets, fixedSize, dCenter, _scale);
	...
	for(int i=0; i<complexPos.size(); i++)
	{
		...
		if(complexPos[i].x()>-999.99)
		{
				...
				{  		
					...
					complexRect[i] = getNodetoBePaint(_boundaryWidth, _fontSize, _insideColor, _boundaryColor, _fontColor, _fontRect, complexPos[i], _scene->_complexNameD[dataID][i][0], 'C', i, sharedSets, expressedSets, fixedSize, dCenter, _scale);	        
	...
	for(int i=0; i<reactionPos.size(); i++)
	{
		if(reactionPos[i].x()>-999.99)
		{
			...
			temp2[0]=m_pathwayID, temp2[1]=temp[0], temp2[2]=temp[1];
			vector<vector<int>> sEdge=findSpannedEdge(scene->edgeconnections, temp2);	
			if(reactionRecord.find(temp)!=reactionRecord.end()||!sEdge.empty())
			{
				...
				reactionRect[i] =getNodetoBePaint( _boundaryWidth, _fontSize, _insideColor, _boundaryColor, _fontColor, _fontRect, reactionPos[i], _scene->_reactionNameD[dataID][i][1], 'R', i, sharedSets, expressedSets, fixedSize, dCenter, _scale, _scene->_reactionNameD[dataID][i][0] );			
			    boundaryWidth[ocount] = _boundaryWidth, fontSize[ocount] = _fontSize, insideColor[ocount] =_insideColor, boundaryColor[ocount] =_boundaryColor, fontColor[ocount] =_fontColor, fontRect[ocount] = _fontRect;
			}
			else
	...
	for(int i=0; i<smallMoleculePos.size(); i++)
	{
		if(smallMoleculePos[i].x()>-999.99)
		{
			...
			if( (!toplevel || reactionRecord.find(temp)!=reactionRecord.end()|| !sEdge.empty()) )	
			{
				 ...
				 smallMoleculeRect[i]=getNodetoBePaint( _boundaryWidth, _fontSize, _insideColor, _boundaryColor, _fontColor, _fontRect, smallMoleculePos[i], _scene->_smallMoleculeNameD[dataID][i][0], 'S', i, sharedSets, expressedSets, fixedSize, dCenter, _scale);			
				 boundaryWidth[ocount] = _boundaryWidth, fontSize[ocount] = _fontSize, insideColor[ocount] =_insideColor, boundaryColor[ocount] =_boundaryColor, fontColor[ocount] =_fontColor, fontRect[ocount] = _fontRect;
				 avaSize = avaSize + _fontSize;
				 if(fontSize[ocount]>maxFontSize) 
					 maxFontSize=fontSize[ocount];
	...
	for(int i=0; i<DnaPos.size(); i++)
	{
		...
		{   
			...
			{	
				...
				DnaRect[i] = getNodetoBePaint(_boundaryWidth, _fontSize, _insideColor, _boundaryColor, _fontColor, _fontRect, DnaPos[i], _scene->_DnaNameD[dataID][i][0], 'D', i, sharedSets, expressedSets, fixedSize, dCenter, _scale);			
				boundaryWidth[ocount] = _boundaryWidth, fontSize[ocount] = _fontSize, insideColor[ocount] =_insideColor, boundaryColor[ocount] =_boundaryColor, fontColor[ocount] =_fontColor, fontRect[ocount] = _fontRect;
				avaSize = avaSize + _fontSize;
				if(fontSize[ocount]>maxFontSize) 
				    maxFontSize=fontSize[ocount];
	...
	{	
		if(physicalEntityPos[i].x()>-999.99)
	    {
			...
			if(!toplevel || reactionRecord.find(temp)!=reactionRecord.end()||!sEdge.empty())
			{
				...
				physicalEntityRect[i]=getNodetoBePaint( _boundaryWidth, _fontSize, _insideColor, _boundaryColor, _fontColor, _fontRect, physicalEntityPos[i], _scene->_physicalEntityNameD[dataID][i][0], 'E', i, sharedSets,expressedSets, fixedSize, dCenter, _scale);		  
				boundaryWidth[ocount] = _boundaryWidth, fontSize[ocount] = _fontSize, insideColor[ocount] =_insideColor, boundaryColor[ocount] =_boundaryColor, fontColor[ocount] =_fontColor, fontRect[ocount] = _fontRect;
				avaSize = avaSize + _fontSize;
				if(fontSize[ocount]>maxFontSize) 
				    maxFontSize=fontSize[ocount];
		  ...
		  if(proteinPos[i].x()>-999.99)
		  {
			  ...
			  if(!toplevel || reactionRecord.find(temp)!=reactionRecord.end()||!sEdge.empty())
			  {
				 // QString s=_scene->_proteinNameD[dataID][i][0];
				  ...
				  proteinRect[i] = getNodetoBePaint(_boundaryWidth, _fontSize, _insideColor, _boundaryColor, _fontColor, _fontRect, proteinPos[i], _scene->_proteinNameD[dataID][i][0], 'P', i, sharedSets, expressedSets, fixedSize, dCenter, _scale);	 		  
				  boundaryWidth[ocount] = _boundaryWidth, fontSize[ocount] = _fontSize, insideColor[ocount] =_insideColor, boundaryColor[ocount] =_boundaryColor, fontColor[ocount] =_fontColor, fontRect[ocount] = _fontRect;
				  avaSize = avaSize + _fontSize;
				  if(fontSize[ocount]>maxFontSize) 
				      maxFontSize=fontSize[ocount];
	...
	{	
		if(ANodePos[i].x()>-999.99)
	    {
			...
			if(!toplevel || reactionRecord.find(temp)!=reactionRecord.end()||!sEdge.empty())
			{
				...
				if(size1>=1 && size2>=1)
				{
					ANodeRect[i]=getNodetoBePaint( _boundaryWidth, _fontSize, _insideColor, _boundaryColor, _fontColor, _fontRect, ANodePos[i], _scene->_ANodeName[m_pathwayID][i][0], 'L', i, sharedSets,expressedSets, fixedSize, dCenter, _scale);		  
					boundaryWidth[ocount] = _boundaryWidth, fontSize[ocount] = _fontSize, insideColor[ocount] =_insideColor, boundaryColor[ocount] =_boundaryColor, fontColor[ocount] =_fontColor, fontRect[ocount] = _fontRect;
					avaSize = avaSize + _fontSize;
					if(fontSize[ocount]>maxFontSize) 
						maxFontSize=fontSize[ocount];
	...
	{	
		if(_pathBubbles[i] && _pathBubbles[i]->isVisible())
		{
			...
			for(int i=0; i<B1->_itemSelected.size(); i+=2)
			{
				...
				switch(type)
				{
					case 'C':   Rect=getNodeRect( B1->complexPos[Cnum], "", type,  fixedSize,  dCenter, _scale);	
						        text =  _scene->_complexNameD[B1->dataID][Cnum][0];
								nodePos = B1->complexPos[Cnum]; 
						        Cnum++; break;
					case 'E':   Rect=getNodeRect( B1->physicalEntityPos[Enum], "", type,  fixedSize,  dCenter, _scale);
						        text = _scene->_physicalEntityNameD[B1->dataID][Enum][0];
								nodePos = B1->physicalEntityPos[Enum]; 
								Enum++; break;
					case 'P':   Rect=getNodeRect( B1->proteinPos[Pnum], "", type,  fixedSize,  dCenter, _scale);	
						        text = _scene->_proteinNameD[B1->dataID][Pnum][0];
								nodePos = B1->proteinPos[Pnum];
								Pnum++; break;
					case 'S': 	Rect=getNodeRect( B1->smallMoleculePos[Snum], "", type,  fixedSize,  dCenter, _scale);	
						        text=_scene->_smallMoleculeNameD[B1->dataID][Snum][0];
								nodePos = B1->smallMoleculePos[Snum]; 
								Snum++; break;
					case 'D':  	Rect=getNodeRect( B1->DnaPos[Dnum], "", type,  fixedSize,  dCenter, _scale);	
						        text = _scene->_DnaNameD[B1->dataID][Dnum][0];
								nodePos = B1->DnaPos[Dnum]; 
								Dnum++; break;
					case 'R':  	Rect=getNodeRect( B1->reactionPos[Rnum], "", type,  fixedSize,  dCenter, _scale);	
						        text= _scene->_reactionNameD[B1->dataID][Rnum][1];
								reactionType=_scene->_reactionNameD[B1->dataID][Rnum][0];
								nodePos = B1->reactionPos[Rnum]; 
								Rnum++; break;
					case 'M': 	Rect=getNodeRect( B1->compartmentPos[id1], "", type,  fixedSize,  dCenter, _scale);	
						        text = _scene->_compartmentName[m_pathwayID][id1][0];
								nodePos = B1->compartmentPos[id1];

								break; 
					/*case 'L': 	Rect=getNodeRect( B1->ANodePos[id1], "", type,  fixedSize,  dCenter, _scale);	
						        text = _scene->_ANodeName[m_pathwayID][id1][0];
								nodePos = ANodePos[id1];
								break; */
				}
...
{    
	...
	if(m_pathwayID>=0)
	{
		...
		{		
			 ...
			 if(!inUpdatedEdge(edge, updatedEdge_4))
			 {
				 ...
				 QColor bc; 
				 if( linkedEdge.size()> m_pathwayID && linkedEdge[m_pathwayID].find(edge)!=linkedEdge[m_pathwayID].end()) //keqin
				 {
				 ...
				 if(pid1==pid2 && pid1==m_pathwayID)
				 {
					 ...
					 drawArrow(painter, start, end, width, height, bc );	
				 }
			 }	
	}
	...
	{//multi-link
			...
			int Aid=-1;
			 ...
			 if(pid1==pid2 && pid1==m_pathwayID)
			 {
				...
				drawCurvedArrow(painter, Code, height/2, start, end, width, height);								
			}
	...
	{
			 tRect.setX(-1000);
			 //if(compartmentPos[i].x()>-999.99)
             if(i<compartmentRect.size() && compartmentRect[i].width()>=0 )
		     {   
				 tRect=paintPreComputedCompartment( painter, boundaryWidth[oCount], fontSize[oCount], insideColor[oCount], boundaryColor[oCount], fontColor[oCount], fontRect[oCount], compartmentRect[i], compartmentPos, _scene->_compartmentName[m_pathwayID], i, _scene->getGroup(this)->sharedSets, fixedSize, dCenter, _scale);	        
				 xlist.insert(int(tRect.x()));
			 }			 
			 oCount++;		    
	}
	if(xlist.size()>1)
	...
	for(int i=0; i<complexPos.size(); i++)
	{
		...
		if(complexRect[i].width()>=0)
		{  		
			tRect= paintPreComputedNode( painter, boundaryWidth[oCount], fontSize[oCount], insideColor[oCount], boundaryColor[oCount], fontColor[oCount], fontRect[oCount], complexRect[i], complexPos[i], _scene->_complexNameD[dataID][i][0], 'C', i, _scene->getGroup(this)->sharedSets, fixedSize, dCenter, _scale);	        
		}	
		oCount++;		
	}	
	temp[0]='R';
	for(int i=0; i<reactionPos.size(); i++)
	{
		tRect.setX(-1000);
		//if(i==16)
		if(reactionRect[i].width()>=0)//&& whichCompartment(m_pathwayID, 'R', i)!=5)
		{
			//if(_scene->_reactionNameD[dataID][i][1].contains("SMAD7") && _scene->_reactionNameD[dataID][i][1].contains("exported"))
			   tRect= paintPreComputedNode( painter, boundaryWidth[oCount], fontSize[oCount], insideColor[oCount], boundaryColor[oCount], fontColor[oCount], fontRect[oCount], reactionRect[i],reactionPos[i], _scene->_reactionNameD[dataID][i][1], 'R', i, _scene->getGroup(this)->sharedSets, fixedSize, dCenter, _scale, _scene->_reactionNameD[dataID][i][0] );			
		}		
		oCount++;
	}
	temp[0]='S';
	for(int i=0; i<smallMoleculePos.size(); i++)
	{
		...
		if(smallMoleculeRect[i].width()>=0)
        {
			tRect= paintPreComputedNode( painter, boundaryWidth[oCount], fontSize[oCount], insideColor[oCount], boundaryColor[oCount], fontColor[oCount], fontRect[oCount], smallMoleculeRect[i], smallMoleculePos[i], _scene->_smallMoleculeNameD[dataID][i][0], 'S', i, _scene->getGroup(this)->sharedSets, fixedSize, dCenter, _scale);
		}		
		oCount++;
	}
	...
	for(int i=0; i<DnaPos.size(); i++)
	{
		...
		if(DnaRect[i].width()>=0)	
		{	
			//tRect=paintNode( painter, DnaPos[i], _scene->_DnaNameD[dataID][i][0], 'S', i, _scene->getGroup(this)->sharedSets, fixedSize, dCenter, _scale);
            //tRect=getNodetoBePaint( painter, boundaryWidth, fontSize, insideColor, boundaryColor, fontColor, DnaPos[i], _scene->_DnaNameD[dataID][i][0], 'S', i, _scene->getGroup(this)->sharedSets, fixedSize, dCenter, _scale);
			tRect= paintPreComputedNode( painter, boundaryWidth[oCount], fontSize[oCount], insideColor[oCount], boundaryColor[oCount], fontColor[oCount], fontRect[oCount], DnaRect[i], DnaPos[i], _scene->_DnaNameD[dataID][i][0], 'D', i, _scene->getGroup(this)->sharedSets, fixedSize, dCenter, _scale);
		}	
		oCount++;
		//DnaRect.push_back(tRect);
	}
	...
	for(int i=0; i<physicalEntityPos.size(); i++)
	{
		  ...
		  if( physicalEntityRect[i].width()>=0)
		  {
			  tRect= paintPreComputedNode( painter, boundaryWidth[oCount], fontSize[oCount], insideColor[oCount], boundaryColor[oCount], fontColor[oCount], fontRect[oCount],  physicalEntityRect[i], physicalEntityPos[i], _scene->_physicalEntityNameD[dataID][i][0], 'E', i, _scene->getGroup(this)->sharedSets,fixedSize, dCenter, _scale);			  
		  }
		  oCount++;		  
	}	
	temp[0]='P';
	for(int i=0; i<proteinPos.size(); i++)
	{
		  ...
		  if(proteinRect[i].width()>=0)
		  {
			  tRect= paintPreComputedNode( painter, boundaryWidth[oCount], fontSize[oCount], insideColor[oCount], boundaryColor[oCount], fontColor[oCount], fontRect[oCount], proteinRect[i], proteinPos[i], _scene->_proteinNameD[dataID][i][0], 'P', i, _scene->getGroup(this)->sharedSets, fixedSize, dCenter, _scale);	 
		  }
		  oCount++;	  
	}
	temp[0]='L';
	for(int i=0; i<ANodePos.size()&& i<_scene->_ANodeName[m_pathwayID].size(); i++)
	{
		  temp[1]=i; 	    
		  if( ANodeRect[i].width()>=0)
		  {
			  ...
			  if(size2>0)
			  {
			      ...
			      tRect= paintPreComputedNode( painter, boundaryWidth[oCount], fontSize[oCount], insideColor[oCount], boundaryColor[oCount], fontColor[oCount], fontRect[oCount],  ANodeRect[i], ANodePos[i], _scene->_ANodeName[m_pathwayID][i][0], 'L', i, _scene->getGroup(this)->sharedSets,fixedSize, dCenter, _scale);			  
			  }
		  }
		  oCount++;		  
	}		
...
{
	//float width= this->Width()/2-46; //_treeRing->GetRingWidth() * _treeRing->_ring.size() * 2.1; //4.5; //2.1
	float height= this->Height()/2-25;
	int itv=20;
	int sx= this->Width()/2-119, sy=height;
	QFont f1("Arial", 10);
	painter->setFont (f1);	
	QString oName, eName;
	int addLength=0;
    if(_orthologyFileName.size()>0)
	{
		...
		float tx = this->Width()/2 -rect3.width()-2;
		if(tx<sx)
			sx=tx;
		addLength++;
	}
	if(_expressionName.size()>0)
	{
		//eName = "Expression:";					
		eName = _expressionName;	
		QFontMetrics fontMetrics(f1); 
        QRectF rect3 = fontMetrics.boundingRect(0, 0, 200, 0, Qt::TextExpandTabs|Qt::TextWrapAnywhere, eName); 
		float tx = this->Width()/2 -rect3.width()-2;
		if(tx<sx)
			sx=tx;
		addLength++;
	...
	painter->drawRect(QRect(sx-6, sy-itv*(4.2 + addLength*1.6 + 1.8), this->Width()/2-sx, 19+itv*(4.2 + addLength*1.6 + 1.8)));

    painter->setPen(QColor(50,50,50,255));	
	
	int grad=7;
	{
		//painter->drawText(sx+10, sy-itv*(grad-0.55), "Bar Chart");
		for(int i=0; i<grad; ++i)
		{
			...
			if(i==6)
			{
				//painter->setBrush(QColor(0, 210, 50, 255)); //htColor[l] = QColor(81,81,255,255);htColor[l] = QColor(230,68,113,255);QColor(255,100,100,255);QColor(206,206,206,228);
			...
			else if(i==2)
			{
				...
				/*if(oName.size()>0)
				{
					painter->drawText(sx+20, sy-itv*(i-0.85), oName); 	
					painter->drawText(sx+20, sy-itv*(i-0.25), str);
				}
				else */
			...
			{   
			   ...
			   //QString str= QString::number((i+1)/8.0, 'g', 5);
				...
				if(i==0) 
				{
					...
					{
						painter->drawText(sx, sy-itv*(7.2-0.95), eName); 	
						str= "Expression:";
						painter->drawText(sx, sy-itv*(7.2-0.25), str);

						painter->drawText(sx, sy-itv*(8.8-0.95), oName); 	
						str= "Orthology:";
						painter->drawText(sx, sy-itv*(8.8-0.25), str);
					}
					...
					else if(oName.size()>0)
					{
						painter->drawText(sx, sy-itv*(7.2-0.95), oName); 	
						str= "Orthology:";
						painter->drawText(sx, sy-itv*(7.2-0.25), str);
					}						
				}
...
{
	/*QRectF box=QRectF(-100,-100,200,200);
	...
	{	
		if(insideBubble(event->pos()))
	    {
			if(!groupSelected) 
			{   
				//if(sharedProtein.empty() && sharedComplex.empty() && sharedSmallMolecule.empty() && sharedPhysicalEntity.empty() && sharedDna.empty() && sharedReaction.empty())
				//highlighted.clear();
				...
				else 
				{
					...
					if(!itemSelected.empty() && onNoteMark(m_pathwayID,  itemSelected[0], itemSelected[1], event->scenePos()))
					{
						vector<QString> text;
						vector<int> Cid;
						_scene->openNodeNote(m_pathwayID, itemSelected[0], itemSelected[1], this, event->scenePos(),text,Cid);
					}
					else
					{
...
int PathBubble1::assignANodeName(QString Name)
{
    int ID=-1,count=0;
	bool start=false;
	while(true)
	{
		...
		{
 			for(int i=toplevel?1:0; i<_scene->_ANodeName[m_pathwayID].size(); i++)
			{
			   if(ANodeRect[i].width()<=0)
			   {
				   ID=i;
		...
		vector<QString> A;
		QString AName='A'+ QString::number(ID);		
...
QRectF PathBubble1::getPos(int type, int id)
{
	switch(type)
	{
		case 'C': return(complexPos[id]); 
		case 'E': return(physicalEntityPos[id]);
		case 'L': return(ANodePos[id]); 
		case 'P': return(proteinPos[id]); 
		case 'S': return(smallMoleculePos[id]); 
		case 'D': return(DnaPos[id]);
		case 'R': return(reactionPos[id]);
		case 'M': return(compartmentPos[id]);
 	}
}
...
bool PathBubble1::aggregation()
{
	...
	QRectF iRect;
	int count=0;
	float maxw=-1000000, maxh=-1000000;
	for(set<vector<int>>::iterator it=highlighted.begin(); it!=highlighted.end(); it++)
	{
        vector<int> item = *it;
		int type=item[0], id=item[1];
	...
	QRectF rect= QRectF(0, 0, maxw, maxh);	
	...
	/*vector<QString> A;
	QString AName='A'+ QString::number(_scene->_ANodeName[m_pathwayID].size());
	A.push_back(AName);  A.push_back(Name);
	_scene->_ANodeName[m_pathwayID].push_back(A);    
	int ID=_scene->_ANodeName[m_pathwayID].size()-1;*/
	int ID = assignANodeName(Name);
	...
	/*for(set<vector<int>>::iterator it=highlighted.begin(); it!=highlighted.end(); it++)
	{
        vector<int> item= *it;
	    int type=item[0], id=item[1];
		_scene->_ANodeContain[m_pathwayID][_ANode.size()-1].push_back(type);
		_scene->_ANodeContain[m_pathwayID][_ANode.size()-1].push_back(id);
	}*/
...
void PathBubble1::updateSpanEdgesWhenCollapse(int Aid)
{
	...
	if(_pathBubbleParent != NULL)
	{
		...
		if(aid1 != -1)
		{
			...
			for(int i=0; i<_scene->_ANodeContain[pid1][aid1].size(); i=i+2)
			{        
					int type = _scene->_ANodeContain[pid1][aid1][i], id = _scene->_ANodeContain[pid1][aid1][i+1];		
					bool flag=false;
...
{	
	...
	for(int i=0,j=0; i<_scene->_ANodeContain[m_pathwayID][ID].size(); i=i+2,j=j+1)
	{
		...
		int cid1=getOriNodeID(_pathBubbleParent->m_pathwayID, m_pathwayID, 'L', Cid);
		if(cid1 != -1)
		{
			vector<vector<int>> tEdge = _pathBubbleParent->edge;		
...
void PathBubble1::updateEdgeAfterCollapse(int Aid)
{
	...
	{        
		...
		switch(type)
		{
		   	case 'C': complexPos[id] = pos;  break;
		    case 'E': physicalEntityPos[id] = pos;	break; 
			case 'L': ANodePos[id] = pos;  break; 
		    case 'P': proteinPos[id] = pos; break; 
		    case 'S': smallMoleculePos[id] = pos; break; 
		    case 'D': DnaPos[id] = pos;   break;  
		    case 'R': reactionPos[id] = pos; break; 
			case 'M': compartmentPos[id] = pos;  break; 
		}	
...
bool PathBubble1::collapse(int Aid)
{
	...
	/*set<vector<int>> eRecord;	
	for(int i=0; i<_scene->_ANodeContain[pid1][aid1].size(); i=i+2)
	 {
		 	 int type = _scene->_ANodeContain[pid1][aid1][i], id = _scene->_ANodeContain[pid1][aid1][i+1];	
			 start[0] = type, start[1] = id;
			 eRecord.insert(start);			 
	 }*/

	

	//update _reaction, _complex,....
	updateEdgetoNode(edge);
		...
		else if( (groupSelected && !_grabisOn&&!_queryisOn&&!_upisOn&&!_downisOn&&!_evenisOn) || (draggingItem && !groupSelected && !_grabisOn && dragout==2) )
		{
			...
			if(!this->realRect().contains(event->pos().x(),event->pos().y()) && varifyItemSelected(itemSelected))
		    {
			   ...
			   iPos=rect.center();
					...
					//caculate the dragged center at the original bubble
					QPointF center(0,0);
					for(int j=0; j<itemSelected.size(); j=j+2)
					{
						...
						switch(type)
						{
							case 'C': center=center+complexRect[id].center();       break;
							case 'E': center=center+physicalEntityRect[id].center();break;
		                    case 'P': center=center+proteinRect[id].center();       break;
		                    case 'S': center=center+smallMoleculeRect[id].center(); break;
		                    case 'D': center=center+DnaRect[id].center();           break;
							case 'R': center=center+reactionRect[id].center();      break;
							case 'M': center=center+compartmentRect[id].center();   break;
							case 'L': center=center+ANodeRect[id].center();   break;

					    }			 					 
		           }
				   if(itemSelected.size()>0)
				        iPos=QPointF(center.x()/itemSelected.size()*2, center.y()/itemSelected.size()*2);	
					

			/*PathBubble1 *ptmp = dynamic_cast<PathBubble1 *>(nitem);
			ptmp->pwdParser->readOrthologySymbol(_treeRing->getOrthFileName(), ptmp->orthologySymbol);
			ptmp ->setOrthFileName(_treeRing->getOrthFileName());
			ptmp->orthology = ptmp->pwdParser->matchSymbol(ptmp->orthologySymbol,  _scene->_proteinNameD[nitem->dataID]);					
			ptmp->drawOrthology = (EDGE_BY==EDGE_BY_ORTHOLOGY);	*/

			   iPos=event->scenePos();
			   subPathBubble1 *rb = dynamic_cast <subPathBubble1*> (_open->openSubPathBubble1(getOrthFileName(), m_pathwayID, realRect().width(), realRect().height(), iPos.x(), iPos.y(), itemSelected, edgeSelected, name));		
		       if ( rb ) 
 	           {
				   ...
				   //need to copy rank to the subpathway
				   if(toplevel)
				   {
					   //RemoveDisabledNodes(_whole, _VertexInfo, _Vertex, Rank0, Ranks);	
	                   //HierarchicalTreeNodeLocation(_whole, _VertexInfo, Rank0, Ranks);
...
{	//arrowType 0: arrow, 1: activation, 2: inhibition
	...
	if(arrowType==0)
		drawArrow(painter, p1+Dis, p2+Dis, width, height, color );	
	else if(arrowType==1) 
		drawArrow_4(painter, p1+Dis, p2+Dis, width, height, QColor(0, 210, 50, 255) );	
	else if(arrowType==2) 
		drawArrow_4(painter, p1+Dis, p2+Dis, width, height, QColor(255, 128, 0, 255) );	
	else if(arrowType==3) 
	{
...
void PathBubble1::drawSharedPointers(QPainter *painter, QPointF dis, vector<int> item1, vector<int> item2)
{
	...
	switch(type2)
	{
		...
		case 'M': itemPos2 = QRectF(compartmentRect[id2].x(),compartmentRect[id2].y(),2,2);  break;
...
void PathBubble1::drawConnections(QPainter *painter, QPointF dis)
{
	...
	/*for (set<vector<vector<int>>>::iterator it = _scene->edgeconnections_A.begin(); it != _scene->edgeconnections_A.end(); it++)
	{
		vector<vector<int>> link = *it;	
	     connections.insert(link);
	}
	for (set<vector<vector<int>>>::iterator it = _scene->edgeconnections_I.begin(); it != _scene->edgeconnections_I.end(); it++)
	{
		vector<vector<int>> link = *it;	
	     connections.insert(link);
	}*/
	for(int i=0; i<4; i++)
	{
		...
		for (set<vector<vector<int>>>::iterator it = connections.begin(); it != connections.end(); it++)
		{
			...
			//rEdge.push_back(link1);  rEdge.push_back(link0);
			...
			QPointF center1, center2;
			//QRectF rect2 = transRect(itemPos2, path2->_scale, path2->dCenter, path2->realRect().width(), path2->realRect().height());
			//QRectF rect1 = transRect(itemPos1, path1->_scale, path1->dCenter, path1->realRect().width(), path1->realRect().height());
			if(itemPos1.width()>0 && itemPos2.width()>0)
			{
				center1 = itemPos1.center();
				center2 = itemPos2.center();
				if(i<3)
				{
					if(!inUpdatedEdge(link, _scene->edgeconnections_4))
					{
						...
						/*if(_scene->edgeconnections_A.find(link)!=_scene->edgeconnections_A.end())
							drawABubbleConnectioninArrow(painter, center1, path1, center2, path2, dis, bc, 1);
						else if(_scene->edgeconnections_I.find(link)!=_scene->edgeconnections_I.end())
							drawABubbleConnectioninArrow(painter, center1, path1, center2, path2, dis, bc, 2);
						else
							drawABubbleConnectioninArrow(painter, center1, path1, center2, path2, dis, bc, 0);	*/
					}
				}
				else
				{
					 //if(pid1==link0[0], pid2=link1[0], type1=link0[1], type2=link1[1], id1=link0[2], id2=link1[2])
					 float h;
					 int encode;
					 if( type1 == 'L')
...
{//record new connections and update the previous ones
	...
	/*for(set<vector<vector<int>>>::iterator it=newlinkVector.begin(); it!=newlinkVector.end(); it++)
	{
		vector<vector<int> > edge=*it,redge;
		redge.push_back(edge[1]); redge.push_back(edge[0]);
		if( (linkVector4.find(edge)==linkVector4.end()&& linkVector4.find(redge)==linkVector4.end()) && (alinkVector.find(edge)!= alinkVector.end() || ilinkVector.find(edge)!= ilinkVector.end() || alinkVector.find(redge)!= alinkVector.end() || ilinkVector.find(redge)!= ilinkVector.end() || newlinkVector.find(redge)!= newlinkVector.end()))
		{
			//linkVector5.insert(*it);
			linkVector4.insert(edge);
		}	
	}*/
...
void PathBubble1::PathBubbleDelectionUpdate_1(int m_pathwayID)
{
	if(_pathBubbleParent==NULL || _pathBubbleParent->m_pathwayID<0)
		return;
	...
	for(int i=0; i<B1->_itemSelected.size(); i+=2)
	{
		...
		switch(type)
		{
		    case 'C': B1->complexPos[Cnum].setX(-1000);         id1=Cnum++;  break;
		    case 'E':  B1->physicalEntityPos[Enum].setX(-1000); id1=Enum++;  break;
			case 'L': B1->ANodePos[Pnum].setX(-1000);            id1=Pnum++;  break;			
		    case 'P': B1->proteinPos[Pnum].setX(-1000);         id1=Pnum++;  break;				
		    case 'S': B1->smallMoleculePos[Snum].setX(-1000);   id1=Snum++;  break;
		    case 'D':  B1->DnaPos[Dnum].setX(-1000);            id1=Dnum++;  break;
		    case 'R': B1->reactionPos[Rnum].setX(-1000);        id1=Rnum++;  break;
		}		
...
{//restore connection etc.
	...
	for(int i=0; i<B1->_itemSelected.size(); i+=2)
	{
		...
		switch(type)
		{
		    case 'C': B0->complexPos[id1]=B1->complexPos[Cnum];
					  Cnum++; break;
		    case 'E': B0->physicalEntityPos[id1] = B1->physicalEntityPos[Enum];  			
					  Enum++; break; 
			case 'L': B0->ANodePos[id1] = B1->ANodePos[Anum]; 
					  Anum++; break; 
		    case 'P': B0->proteinPos[id1] = B1->proteinPos[Pnum]; 
					  Pnum++; break; 
		    case 'S': B0->smallMoleculePos[id1] = B1->smallMoleculePos[Snum];    
					  Snum++; break; 
		    case 'D':  B0->DnaPos[id1] = B1->DnaPos[Dnum];  
					  Dnum++; break;  
		    case 'R': B0->reactionPos[id1] = B1->reactionPos[Rnum]; 
					  Rnum++; break; 
			case 'M': B0->compartmentPos[id1] = B1->compartmentPos[id1]; 
					  break; 
		}			
	}	
	...
	for(int i=0; i<B1->_itemSelected.size(); i+=2)
	{
		...
		switch(type)
		{
		    case 'C': B1->complexPos[Cnum].setX(-1000);         id1=Cnum++;  break;
		    case 'E':  B1->physicalEntityPos[Enum].setX(-1000); id1=Enum++;  break;
			case 'L': B1->ANodePos[Anum].setX(-1000);           id1=Anum++;  break;				
		    case 'P': B1->proteinPos[Pnum].setX(-1000);         id1=Pnum++;  break;				
		    case 'S': B1->smallMoleculePos[Snum].setX(-1000);   id1=Snum++;  break;
		    case 'D':  B1->DnaPos[Dnum].setX(-1000);            id1=Dnum++;  break;
		    case 'R': B1->reactionPos[Rnum].setX(-1000);        id1=Rnum++;  break;
		...
		linkRecord.clear();
		//set<vector<vector<int>>>::iterator it;
		for(it = _scene->edgeconnections_A.begin(); it != _scene->edgeconnections_A.end(); it++)
		{
		     vector<vector<int>> plink = *it;
			 //vector<vector<int>>::iterator id=plink.find(link1);
		     //if(id!=plink.end())
		...
		{
			 vector<vector<int>> plink=linkRecord[j];
			 _scene->edgeconnections_A.erase(plink);			  
			 ...
			 if(od!=-1)
			 {
				 ...
				 if(link[0]!=link2[0]) 
				 {
					 //plink.insert(link); //add the oriental item back and connected with link2
					 plink[1-od]=link;
					 _scene->edgeconnections_A.insert(plink);
				 }
			 }
		}	
	}
...
{	
	...
	{	
		...
		else if(_grabisOn||_queryisOn||_upisOn)//groupSelected
	    {
			if(outsideBubble(event->pos()))
			{
				if(bubbleRect.width()==0)
				{
				    ...
				    bubbleRect.setWidth(200), bubbleRect.setHeight(200/1.618);								
				}
			}
			else 
			{		
		...
		else
		{
		    ...
		    {			
				bool flag1=false;
				if(varifyItemSelected(itemSelected) || insideBubble(event->pos()) )
				{
			...
			if(outsideBubble(event->pos()))
			{
				if( (itemSelected[0]!= 'M' && getVisibleItemNum()>1) || (itemSelected[0]== 'M' && getVisibleCompartmentNum()>1) )					
				{
					bubbleRect.setWidth(200), bubbleRect.setHeight(200/1.618);
					bubbleRect.moveCenter(event->pos());
					dragout=2;
...
{	
    ...
    switch(type)
	{
	    ...
	    case 'M': updateItemGrid(itemGrid, type, id, move+compartmentPos[id].center());  			
			      updateACompartmentPos(id, move, Move); break; //if(nodeNote!=NULL)  nodeNote->mySetPos(move+nodeNote->pos()); 			      
	}	
	return true;
...
void PathBubble1::PL(vector<PathBubble1 *> mlist)
{
	...
	{
		PathBubble1* pbubble = mlist[i];		
		pbubble->searchLinkedNode( 1, 0, 0);   	
		for(int j=0; j<pbubble->sharedProtein.size(); j++)
		{
			pbubble->sharedProtein[j].clear();
			pbubble->sharedSmallMolecule[j].clear();
...
void PathBubble1::CL(vector<PathBubble1 *> mlist)
{
		...
		{
			PathBubble1* pbubble = mlist[i];
			pbubble->searchLinkedNode( 0, 1, 0);   		
...
{   //return weather graph changed
	...
	{   //P U
		...
		sharedDifferedLinkedExpressedtoHighlighted();
...
{    	
	...
	else if(index==0) //load expression	
	{
	   //load
		...
		if( name.size() > 0 )
		{
			ItemBase* item;
			item = NULL;				
			_scene->getOpenManager()->openExpressionByPath(item, 300, scenePos.x(), scenePos.y(), name);
	...
	else if(index==1) //show expressed gene
	{
		...
		sharedDifferedLinkedExpressedtoHighlighted(false);
	...
	//else if (selectedAction == resetAction)
	{
       ...
       _scene->resetItemPos(m_pathwayID, complexPos, proteinPos, smallMoleculePos, DnaPos, EmptyPos, reactionPos, physicalEntityPos, compartmentPos, ANodePos, edge, Rank0, Ranks, _newPos, _newCompartmentPos, _scale, dCenter, drawSmallmolecule);
	   _scale=_ScaleBackup;
	   x0y0=QPoint(-this->Width()/2.0,-this->Height()/2.0);	
	   x1y1=QPoint(this->Width()/2.0,this->Height()/2.0);
	   _penisOn=false;	
	...
	{   
		...
		QString fileName = QFileDialog::getSaveFileName(0, tr("Save Pathway Graph as xml file"), pathName,".xml (*.xml)");//"c:\\file.x", "X files (*.x)"
        //savePathWayforTreeRing(fileName, edge);
		savePathWay(fileName, complexPos, proteinPos, smallMoleculePos, DnaPos, reactionPos, physicalEntityPos, ANodePos, edge);
...
{   //save edge and node (not include position)//similar to VTK style
	...
	Estart=count;
	for (int j = 1; j < _scene->_physicalEntityNameD[dataID].size(); j++)
	{
	...
	Rstart=count;
	for (int j = 1; j < _scene->_reactionNameD[dataID].size(); j ++)
	{
...
{   //save edge and node (not include position)//similar to VTK style
	...
	for (int j = 1; j < _scene->_physicalEntityNameD[dataID].size(); j++)
	{
        //if(physicalEntityRect[j].width()>=0)//for big bubble
		{
			vector<int> node; 	node.push_back('E'); node.push_back(j);							
			for(int i=0; i< _scene->_compartmentName[m_pathwayID].size()-1; i++)
			{		
	...
	for (int j = 1; j < _scene->_smallMoleculeNameD[dataID].size(); j ++)
	{
		//if(smallMoleculeRect[j].width()>=0)//for big bubble
		{
			vector<int> node; node.push_back('S'); node.push_back(j);
			for(int i=0; i< _scene->_compartmentName[m_pathwayID].size()-1; i++)
			{	
...
{   //save edge and node (not include position)//similar to VTK style
	...
	;
	str += "VERTICES ";			
	tmpStr.setNum( _scene->_physicalEntityNameD[dataID].size() -1 
		           +  _scene->_complexNameD[dataID].size()-1 
				   +  _scene->_proteinNameD[dataID].size()-1
				   +  _scene->_smallMoleculeNameD[dataID].size()-1
				   +  _scene->_DnaNameD[dataID].size()-1
				   +  _scene->_reactionNameD[dataID].size()-1
				   );
	str += tmpStr;
	str += "\n";
...
void PathBubble1::savePathWay(QString filename, vector<QRectF> complexPos, vector<QRectF> proteinPos, vector<QRectF> smallMoleculePos, vector<QRectF> DnaPos, vector<QRectF> reactionPos, vector<QRectF> physicalEntityPos, vector<QRectF> ANodePos, vector<vector<int>> edge)
{
	/*QString inforstr = "Save the current scene as the xml file ";
	inforstr.append( filename );
	inforstr.append( "\n" );
	...
	compartmentNum = compartmentNum<_scene->_compartmentName.size()?_scene->_compartmentName.size():compartmentNum;
	...
	for (int j = 0; j < compartmentNum; j ++)
	{
		...
		QString cstr(_scene->_compartmentName[m_pathwayID][j][0]); 
...
//complex
	...
	for (int j = 1; j <_scene->_complexNameD[dataID].size(); j ++)//complexNum+1
	{
...
//protein		
	...
	for (int j = 1; j < _scene->_ANodeName[m_pathwayID].size(); j ++)
	{
		...
		//////
		QDomElement parePos = doc.createElement( tr("Name") );
		QString cstr(_scene-> _ANodeName[m_pathwayID][j][0]); 
...
//smallMolecule		
	...
	for (int j = 1; j < reactionPos.size(); j ++)
	{
...
bool PathBubble1::readPathWay(QString filename, vector<QRectF> &complexPos, vector<QRectF> &proteinPos, vector<QRectF> &smallMoleculePos, vector<QRectF> &DnaPos, vector<QRectF> &reactionPos, vector<QRectF> &physicalEntityPos, vector<QRectF> &ANodePos, vector<vector<int>> &edge)
{
	...
	//edge.clear(); //currently ignore edge
	
	//_compartmentPos.push_back(rect);
	...
	for(int i=0; i<ANodePos.size() && i<_ANodePos.size(); i++)
	{
...
{		
	...
	//if(toplevel)
		...
		/*if(_scene->_compartmentName[m_pathwayID][_scene->_compartmentName[m_pathwayID].size()-1][0]=="whole")
			_scene->_compartmentName[m_pathwayID].resize(_scene->_compartmentName[m_pathwayID].size()-1);
		*/
		vector<QRectF> reactionPos1 = reactionPos;					
		stepTracked.clear();			
		_scene->CompartmentContain[m_pathwayID]=CompartmentContain;	
		
		if( _pathName=="Urea cycle" || _pathName=="Acetylcholine Binding And Downstream Events" || _pathName== "Acetylation" || !pathWayRead(_whole, _VertexInfo, _Vertex, Rank0, Ranks,_newPos,_newCompartmentPos, _Edge, true) || !pathWayPoiRead(_whole, complexPos_1, proteinPos_1, smallMoleculePos_1, DnaPos_1, EmptyPos_1, reactionPos_1, physicalEntityPos_1, compartmentPos_1, ANodePos_1, CompartmentContain_1, _scale, dCenter, true))
		{
			preComputed=false;
			preLocateNodes(complexPos,proteinPos,smallMoleculePos);		
			ParserTraceSteps();
			...
			HierarchicalTreeNodeLocation(_whole, _VertexInfo, Rank0, Ranks);
			//updateBubbleSize_2();	
			//_scene->backupItemPos(m_pathwayID, complexPos, proteinPos, smallMoleculePos, DnaPos, EmptyPos, reactionPos, physicalEntityPos, compartmentPos, _scale, dCenter, drawSmallmolecule);
			//_scene->backupRanks(m_pathwayID, Rank0, Ranks, _newPos, _newCompartmentPos);					
			putReactionToCompartment(_reaction, _scene->CompartmentContain[m_pathwayID]);	
			removeNotContainedinCompartment(CompartmentContain, compartmentPos, complexPos, proteinPos, smallMoleculePos, DnaPos, reactionPos, degradationPos, physicalEntityPos, EmptyPos, ANodePos);
			//putReactionToCompartment(_reaction, _scene->CompartmentContain[m_pathwayID], Ranks, _VertexInfo);	
			...
			pathWayRecord(_whole, _VertexInfo, _Vertex, Rank0, Ranks,_newPos,_newCompartmentPos, _Edge, true);
			//pathWayPoiRecord(complexPos, proteinPos, smallMoleculePos, DnaPos, EmptyPos, reactionPos, physicalEntityPos, compartmentPos, ANodePos, CompartmentContain, _scale, dCenter, drawSmallmolecule);
			//pathWayPoiRead(_whole, complexPos, proteinPos, smallMoleculePos, DnaPos, EmptyPos, reactionPos, physicalEntityPos, compartmentPos, ANodePos, CompartmentContain, _scale, dCenter, drawSmallmolecule);
		}
		//test and remove not contained
		//if(preComputed)
		//{
