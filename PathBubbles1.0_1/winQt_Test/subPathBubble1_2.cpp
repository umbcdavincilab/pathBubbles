...
subPathBubble1::subPathBubble1(QString orthName, int pathwayID, int size, int sizey, int x, int y, vector<int> ItemSelected, vector<vector<int>> EdgeSelected, OpenGLScene* Scene, StatisticManager* manager, OpenManager* open, QString name, QString pathName, QString lastStoredGraph)
:  PathBubble1 (pathwayID, size, x, y, Scene, manager, open, name, orthName, pathName,lastStoredGraph)
{
    ...
    {		 
		...
		newHeight = newHeight + 16; //space for legend
		if(newHeight<170)
			newHeight = 170;
		newWidth = newWidth +150; //space for legend
		...
		float S=-10000;
		reScale(x0y0, x1y1, QPointF(-58,0), QPointF(-58,0), S, dCenter);	
	...
	//if(complexPos.size()==0)
	{
		/*complexPos=_scene->_complexPos[m_pathwayID];
		proteinPos=_scene->_proteinPos[m_pathwayID];
		smallMoleculePos=_scene->_smallMoleculePos[m_pathwayID];
		DnaPos=_scene->_DnaPos[m_pathwayID];
		reactionPos=_scene->_reactionPos[m_pathwayID];
		physicalEntityPos=_scene->_physicalEntityPos[m_pathwayID];
		ANodePos=_scene->_ANodePos[m_pathwayID];*/

		itemGrid=recordItemGrid(visibleItemNum, visibleCompartmentNum);	

		if(toplevel)
		{
			complexNum=complexPos.size()-1; 	
			reactionNum = reactionPos.size()-1;		
			proteinNum = proteinPos.size()-1;
			smallMoleculeNum = smallMoleculePos.size()-1;
			DnaNum = DnaPos.size()-1;
			physicalEntityNum = physicalEntityPos.size()-1;	   
		}
		else
        {
	...
	QPointF ocenter = (x1y1 + x0y0) / 2.0;
    
	initWidth = newWidth; //fabs(x1y1.x()-x0y0.x());//fabs(X1Y1.x()-X0Y0.x());//newWidth;//fabs(X1Y1.x()-X0Y0.x());
	x1y1=ocenter + QPointF(newWidth/2.0,newHeight/2.0);
	x0y0=ocenter - QPointF(newWidth/2.0,newHeight/2.0);

	reScale(x0y0, x1y1, -ocenter-center,  -ocenter-center, S, dCenter);		
	moveGraphToBePaint( -ocenter-center);	*/
	
	//for actually move pos for each node
	x1y1 = QPointF(newWidth/2.0,newHeight/2.0);
	x0y0 = - QPointF(newWidth/2.0,newHeight/2.0);
	...
	/*QPointF tcenter= QPointF(dCenter.x()*newWidth, dCenter.y()*newHeight);
	//QPointF tcenter = (x0y0+x1y1)/2;
	//   QPointF dis=x1y1-x0y0;
	x0y0 = tcenter - QPointF(newWidth/2,newHeight/2); 
	x1y1 = tcenter + QPointF(newWidth/2,newHeight/2); */
	/*QPointF center = (x1y1 + x0y0) / 2.0;	
    dCenter.setX(center.x()/this->Width());
	dCenter.setY(center.y()/this->Height());*/
	...
	{		
		...
		*/
		...
		compartmentPos.resize(compartmentPos.size()+1);
...
QRect subPathBubble1::updateBubbleSize_3()
{
	...
	dCenter = _pathBubbleParent->dCenter;
...
void subPathBubble1::subPathWay(vector<int> ItemSelected, vector<vector<int>> EdgeSelected)
{
				...
				if(type!='M')
				{
					...
					{ 
						case 'C': if(nodeNote!=NULL)  nodeNote->_pid=_pid, nodeNote->_type=type, nodeNote->_id=_scene->_complexPos[pathwayID].size();
								  ...
								  _scene->_complexContain[pathwayID].push_back(_scene->_complexContain[m_pathwayID][id]);	
	...
	initItemSize(ItemSelected, mflag);//get new x0y0 x1y1
...
void subPathBubble1::initItemSize(vector<int> ItemSelected, bool mflag)
{
	//vector<int> item(2,0);
	int x0,y0,x1,y1;
	...
	for(int i=0; i<ItemSelected.size(); i+=2)
	{
		...
		if( !(type=='M' && virtualCompartment.find(id)!=virtualCompartment.end()) ) 		
		{
			...
			node[0]=type, node[1]=id, node[2]=-1, node[3]=-1, node[4]=-1;
			if(type!='M') //keqin
