...
void FileItem::drawName( QPainter* painter )
{
	...
	painter->drawText( this->boundingRect().bottomLeft(), result );
	painter->restore();
	return;
}
...
void FileItem::disablePreOpen( QPointF pos, bool open )
{
	if( !open )
	{
		//this->_open->openDataInViewAt( this->_name, removeOpened( _view ), pos );  // keqin changed to allow repeat drag a composited rect out of control panel
		this->_open->openDataInViewAt( this->_name, _view, pos );  
	}
	this->PREOPEN = false;
	this->SIMUOPEN = false;
	this->_preRect = this->boundingRect();
...
{
	QList<int> result;
	/*for( int i = 0; i < _openedView.size(); i++ )
	{
			int ov=_openedView[i];
			ov=ov;
	}*/
