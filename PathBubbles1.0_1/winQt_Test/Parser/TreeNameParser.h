...
/************************************************************************
  ...
  list. and change the tree structure to the 3 levels
...
class TreeNameParser
{
  public:
    TreeNameParser(char *filename);
	virtual ~TreeNameParser();
	...
	//     no replication
	vector<int> GetLevelXParCount(int levelIndex);
	// return the counter of each parameters in that level.
  ...
  protected:
	...
	void pushbackLp(int levelIndex, char *token, vector<string> _tmpToken);
	...
	int parCount[MAX_LEVEL];  //作用和tmp_lpc相同
...
///////////// simply pass from the _lev ... leave these here anyway.
    ...
    vector<string> tmp_lp[MAX_LEVEL]; //vector that holds name of level
	// save the levels
    vector<int > tmp_lpc[MAX_LEVEL]; //存储每一层对第i个节点，有多少个相同的节点
