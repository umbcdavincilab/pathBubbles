...
void eQuery::query(const QString &ip)  
{  
    QString strUrl = QString("http://opendata.baidu.com/api.php?query=%1&resource_id=6006&ie=utf8&format=json").arg(ip);  //https://biotm.cis.udel.edu/udelafc/getiTermPage.php?user=liang&pass=iTermPageForLiang&gene=gro"); 
    QUrl url(strUrl);  
    QNetworkRequest req(url);  
    req.setRawHeader("Accept", "text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,*/*;q=0.8");  
    req.setHeader(QNetworkRequest::ContentTypeHeader, "Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/35.0.1916.114 Safari/537.36");  
    QNetworkReply *reply = m_nam.get(req);  
    reply->setProperty("string_ip", ip);  
}  
  
void eQuery::query(quint32 ip)  
{  
    QHostAddress addr(ip);  
    query(addr.toString());  
}  
  
void eQuery::onReplyFinished(QNetworkReply *reply)  
{  
    reply->deleteLater();  
    QString strIp = reply->property("string_ip").toString();  
    if(reply->error() != QNetworkReply::NoError)  
    {  
        qDebug() << "eQuery, error - " << reply->errorString();  
        emit finished(false, strIp, m_emptyString);  
        return;  
    }  
  
    int status = reply->attribute(QNetworkRequest::HttpStatusCodeAttribute).toInt();  
    //qDebug() << "eQuery, status - " << status ;  
    if(status != 200)  
    {  
        emit finished(false, strIp, m_emptyString);  
        return;  
    ...
    {  
        qDebug() << "eQuery, json error - " << err.errorString();  
        emit finished(false, strIp, m_emptyString);  
        return;  
    }  
